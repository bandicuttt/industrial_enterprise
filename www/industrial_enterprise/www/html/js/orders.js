...
function finishOrder(orderId) {
  const apiUrl = `http://127.0.0.1:8000/api/admin/order/${orderId}/`;

  // Create a new date object to set as the delivery date
  const now = new Date();

  // Create the request body
  const data = {
    delivery_date: now.toISOString().split("T")[0], // format the date as yyyy-mm-dd
  };
  ...
  .then((response) => {
    if (response.status === 200) {
      ...
      window.location.href =
        "http://localhost/industrial_enterprise/www/html/auth.html";
    }
  })
  .catch((error) => {
    console.error(error);
    ...
    success: function (data) {
      try {
        if (data.role.title === "Администратор") {
          ...
          searchButton.addEventListener("click", (event) => {
            ...
            if (searchQuery == "" || searchCategory == "Не выбрано") {
                ...
                .then((users) => {
                  ...
                  const filteredUsers = users.filter((user) => {
                    if (searchCategory === "ID") {
                      ...
                      $("#error-message-search")
                        .text("Категория поиска не выбрана!")
                        .css("color", "red");
                      setTimeout(function () {
                        $("#error-message-search").text("").css("color", "");
                      }, 3000);
                  ...
                  <div class="text-center card-box">
                      <div class="member-card pt-2 pb-2">
                          ...
                          <div class="">
                              <h4>ID: ${user.id}</h4>
                              <p class="text-muted">Товар: <span>| </span><span><a href="#" class="text-pink">${
                                user.product.product_name
                              }</a></span></p>
                              <p class="text-muted">Количество <span>| </span><span><a href="#" class="text-pink">${
                                user.volume
                              }</a></span></p>
                              <p class="text-muted">Создан <span>| </span><span><a href="#" class="text-pink">${
                                user.created_at
                              }</a></span></p>
                              <p class="text-muted">Доставлен <span>| </span><span><a href="#" class="text-pink">${status}</a></span></p>
                              <p class="text-muted">Адрес доставки <span>| </span><span><a href="#" class="text-pink">${
                                user.delivery_address
                              }</a></span></p>
                              <p class="text-muted">Контактный телефон <span>| </span><span><a href="#" class="text-pink">${
                                user.customer.phone_number
                              }</a></span></p>
                              <p class="text-muted">Курьер <span>| </span><span><a href="#" class="text-pink">${
                                user.driver.user.first_name +
                                " " +
                                user.driver.user.last_name
                              }</a></span></p>
                              <p class="text-muted">Вес <span>| </span><span><a href="#" class="text-pink">${
                                user.volume * user.product.weight
                              } кг</a></span></p>
                              <p class="text-muted">Стоимость <span>| </span><span><a href="#" class="text-pink">${
                                Number(user.volume) *
                                  Number(user.product.product_price) +
                                Number(user.unit.delivery_price)
                              } $</a></span></p>
                          </div>
                      </div>
                  </div>
              `;
                    ...
                    const driverButtons = document.querySelectorAll(
                      ".btn.btn-primary.mt-3.btn-rounded.waves-effect.w-md.waves-light"
                    );
                    driverButtons.forEach((button) => {
                      button.addEventListener("click", (event) => {
                        ...
                        window.location.href =
                          "http://localhost/industrial_enterprise/www/html/profile_view.html?id=" +
                          ProfileId;
                      });
                    });
                  });
            ...
            .then((data) => {
              ...
              users.forEach((user) => {
                ...
                if (user.product_photo == null) {
                  ...
                  var user_photo =
                    "../../media/media/" + user.product_photo.split("/").pop();
                }
                if (user.delivery_date == null) {
                  var status = "В пути 🕔";
                } else {
          ...
          window.location.href =
            "http://localhost/industrial_enterprise/www/html/profile_menu.html";
        }
      } catch (error) {
        const searchbar = document.querySelector(".searchbar");
        searchbar.style.display = "none";
        document.querySelector("#dob-chart").style.display = "none";
        document.querySelector(".main-breadcrumb").style.display = "none";
        document.querySelector(".main-breadcrumb-driver").style.display =
          "block";
        localStorage.setItem("user_id", data.user.id);
        if (data.user.role.title === "Водитель") {
          const searchButton = document.querySelector("#search_drivers");
          const downloadButton = document.querySelector("#download_drivers");
          downloadButton.addEventListener("click", (event) => {
            event.preventDefault();
            ...
            if (searchQuery == "" || searchCategory == "Не выбрано") {
              ...
              fetch("http://127.0.0.1:8000/api/orders/", {
                ...
                .then((users) => {
                  ...
                  filteredUsers.forEach((user) => {
                    if (user.driver.id == localStorage.getItem("user_id")) {
                      const div = document.createElement("div");
                      div.setAttribute("class", "col-lg-4");
                      const count = document.querySelector(".count_drivers");
                      count.setAttribute(
                      ...
                      <div class="member-card pt-2 pb-2">
                          ...
                          <div class="">
                              ...
                              <button type="button" id="${
                                user.id
                              }" class="btn btn-primary" data-dismiss="modal">Завершить заказ</button>
                              </div>
                      </div>
                  </div>
              `;
